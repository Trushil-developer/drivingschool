...
(async () => {
    ...
    const tabRenderers = {
        ...
        expenses: async () => {
            showLoading();
            try {
                if (typeof window.renderExpensesModule !== "function") {
                    tableWrap.innerHTML = '<div class="error">Expenses module not loaded</div>';
                    return;
                }
                await window.renderExpensesModule(tableWrap);
            } finally {
                hideLoading();
            }
        },
        ...
        appSettings: async () => {
            showLoading();
            try {
                if (typeof window.renderAppSettingsModule !== "function") {
                    tableWrap.innerHTML = '<div class="error">App Settings module not loaded</div>';
                    return;
                }
                await window.renderAppSettingsModule(tableWrap);
            } finally {
                hideLoading();
            }
        },
