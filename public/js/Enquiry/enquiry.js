document.addEventListener("DOMContentLoaded", async () => {
    ...
    const BYPASS_OTP = true; // Email verification commented out – not needed right now
    ...
    document.querySelectorAll(".next-btn").forEach(btn => {
        btn.addEventListener("click", () => {
            ...
            // Email verification commented out – skip OTP check on step 1
            // if (currentStep === 0) {
    ...
    /* =====================================================
    EMAIL VERIFICATION – COMMENTED OUT (not needed right now)
    All OTP-related code is wrapped so it only runs when
    the verification UI elements exist in the DOM.
    ===================================================== */
    if (sendOtpBtn && otpSection && resendBtn) {
        ...
        if (verifyCards.length) {
            verifyCards.forEach(card => {
                card.addEventListener("click", () => {
                    ...
                    verifyMethod = card.dataset.method;
                    otpVerified = false;
                    resendAttempts = 0;
        ...
        sendOtpBtn.addEventListener("click", async () => {
            ...
            if (name.length < 2) { alert("Please enter your full name."); document.getElementById("name").focus(); return; }
            const phone = document.getElementById("phone").value.trim();
            if (!/^[6-9]\d{9}$/.test(phone)) { alert("Please enter a valid 10-digit mobile number."); document.getElementById("phone").focus(); return; }
            ...
            if (verifyMethod === "mobile") { alert("📱 Mobile OTP coming soon. Use email verification."); return; }
            ...
            try {
                ...
                const res = await fetch("/api/enquiries/send-email-otp", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email }) });
        ...
        async function verifyOtp() {
            ...
            try {
                const res = await fetch("/api/enquiries/verify-email-otp", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email, otp }) });
                const data = await res.json();
        ...
        resendBtn.addEventListener("click", async () => {
            if (resendAttempts >= MAX_RESENDS) { alert("Maximum resend attempts reached. Try again later."); return; }
            resendAttempts++;
            resendBtn.disabled = true;
            try {
                const res = await fetch("/api/enquiries/send-email-otp", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email: emailInput.value.trim() }) });
                const data = await res.json();
                if (!data.success) throw new Error();
                otpInputs.forEach(i => i.value = "");
                otpInputs[0].focus();
        ...
        otpInputs.forEach((input, index) => {
            input.addEventListener("input", () => {
                ...
                if (Array.from(otpInputs).every(i => i.value.length === 1)) verifyOtp();
            });
    ...
    enquiryForm.addEventListener("submit", async (e) => {
        ...
        // Email verification commented out – skip OTP check on submit
        ...
        try {
            ...
            emailInput.disabled = false;
            if (sendOtpBtn) { sendOtpBtn.disabled = false; sendOtpBtn.textContent = "Send Verification Code"; }
