...
function showModalAlert(message) {
  ...
  Modal.show();
  setTimeout(() => {
    document.getElementById("modalOk").onclick = () => Modal.hide();
  }, 20);
}
  ...
  win.document.write(`
    <html>
      <head>
        ...
        <style>
          button, #appModal { display:none!important; }
        </style>
      </head>
      <body>
...
form.addEventListener("submit", async (e) => {
  ...
  setTimeout(() => {
    document.getElementById("modalPrint").onclick = () => {
      printFullForm();
      Modal.hide();
      if (window._shouldResetForm) form.reset();
    };
